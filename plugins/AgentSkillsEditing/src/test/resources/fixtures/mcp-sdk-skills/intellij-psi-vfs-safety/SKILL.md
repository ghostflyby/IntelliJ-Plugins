...
需要与 PSI、committed offset、navigation/symbol resolution 保持一致的 Document 读取，必须使用 committed document 并在 read
access 下进行。
...
VFS 读写遵循具体 API 的锁注解（`@RequiresReadLock`、`@RequiresWriteLock`）和文档合同。当需要与 PSI/Document 状态保持一致性快照时，获取对应的
read/write access。
