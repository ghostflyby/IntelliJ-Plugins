...
internal class ResourceRouteSnapshotTest {
        ...
        return ResourceRouteCompiler.compile(
            listOf(
                WorkspaceResourceRouteContribution(
                    featureName = "core",
                    roots = core.roots,
                    resourceListRoutes = core.resourceListRoutes,
                    templateListRoutes = core.templateListRoutes,
                ),
                WorkspaceResourceRouteContribution(
                    featureName = "file-content",
                    roots = fileContent.roots,
                    resourceListRoutes = fileContent.resourceListRoutes,
                    templateListRoutes = fileContent.templateListRoutes,
                ),
            ),
        )
    }
    ...
    private fun fileMatcher(snapshot: ResourceRouteSnapshot): SegmentTreeTemplateMatcher {
        return SegmentTreeTemplateMatcher(
            ResourceTemplate(
                uriTemplate = "ij-workspace://{instanceKey}/projects/{projectKey}/files/{relativePath}{?meta,content,exists,structure,glob}",
                name = "relativePath",
            ),
    ...
    private fun vfsMatcher(snapshot: ResourceRouteSnapshot): SegmentTreeTemplateMatcher {
        return SegmentTreeTemplateMatcher(
            ResourceTemplate(
                uriTemplate = "ij-workspace://{instanceKey}/vfs/{rawVfsUrl}{?meta,content,exists,structure,glob}",
                name = "rawVfsUrl",
            ),
            ResourceRouteSnapshotRef(snapshot),
        )
