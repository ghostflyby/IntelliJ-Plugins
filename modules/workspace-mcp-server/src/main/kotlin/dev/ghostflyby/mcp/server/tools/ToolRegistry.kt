...
public typealias Handler = suspend ClientConnection.(CallToolRequest) -> CallToolResult

private typealias ToolResultMapper = (Any?) -> CallToolResult

public enum class ToolRejectReason {
    WRONG_RECEIVER,
    NO_SCHEMA,
}

public sealed class ToolReflectionResult {
    public data class Accepted(val tool: Tool, val handler: Handler) : ToolReflectionResult()
    public data class Rejected(val reasons: Set<ToolRejectReason>) : ToolReflectionResult()
}

public fun <T : Any> reflectTools(
    toolClass: KClass<T>,
...
public fun buildKotlinReflectHandler(
    instance: Any,
    func: KFunction<*>,
    info: ToolMethodInfo,
    callFactory: McpCallFactory,
...
public data class ToolInvocationPlan(
    public val name: String,
    public val argumentDecoders: List<ToolArgumentDecoder>,
    public val callArgsBuilder: ToolCallArgsBuilder,
    public val methodHandle: MethodHandle,
    public val isSuspend: Boolean,
    public val resultMapper: ToolResultMapper,
)

public data class ToolArgumentDecoder(
    public val name: String?,
    public val serializer: KSerializer<Any?>,
    public val isOptional: Boolean,
    public val valueAdapter: ToolArgumentValueAdapter,
    public val defaultPlaceholder: Any?,
    public val defaultMaskIndex: Int,
    public val defaultMaskBit: Int,
)

public fun interface ToolArgumentValueAdapter {
    public fun adapt(value: Any?): Any?
}

public fun interface ToolCallArgsBuilder {
    public fun build(
...
public data class ToolCallArgs(
    ...
    public val argumentStart: Int,
    public val maskStart: Int?,
)

public fun compileToolInvocationPlan(
    instance: Any,
    func: KFunction<*>,
    callFactory: McpCallFactory,
): ToolInvocationPlan {
...
public fun buildMethodHandleHandler(plan: ToolInvocationPlan): Handler {
    suspend fun handleRequest(conn: ClientConnection, request: CallToolRequest): CallToolResult {
...
public inline fun <reified T> T.toMcpCallToolResult(): CallToolResult {
    return this.toMcpCallToolResult(typeOf<T>())
}

public fun Any?.toMcpCallToolResult(type: KType): CallToolResult {
    return when (this) {
...
public class ToolMethodInfo(
    public val name: String,
    public val paramClasses: List<KType>,
) {
    public companion object {
        public fun classify(func: KFunction<*>): Set<ToolRejectReason> {
