...
public class WorkspaceMcpServerCore(
    private val parentScope: CoroutineScope,
    private val serverInfo: Implementation,
    private val instructions: String,
    private val initialFeatures: List<WorkspaceMcpFeature>,
    private val stateFlows: ResourceChangeNotifier,
    ...
    public val roots: StateFlow<WorkspaceMcpSessionRootsState> get() = sessionRoots.state

    public val server: Server = createServer()
    ...
    public fun hasResourceSubscriptions(): Boolean = sessionState.hasResourceSubscriptions()

    public suspend fun close() {
        server.close()
    }

    public fun register(feature: WorkspaceMcpFeature) {
        featureCoordinator.register(server, feature)
        globalGen.update { it + 1 }
    }

    public fun unregister(featureName: String) {
        featureCoordinator.unregister(server, featureName)
        globalGen.update { it + 1 }
    }

    public fun syncResources() {
...
public interface WorkspaceMcpCoreLogger {
    ...
    public object Noop : WorkspaceMcpCoreLogger {
        override fun warn(message: String, error: Throwable?): Unit = Unit
    }
}
