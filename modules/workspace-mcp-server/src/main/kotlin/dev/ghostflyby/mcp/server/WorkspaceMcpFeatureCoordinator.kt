...
import dev.ghostflyby.mcp.server.route.ResourceRouteCompiler
import dev.ghostflyby.mcp.server.route.ResourceRouteSnapshot
import dev.ghostflyby.mcp.server.route.WorkspaceResourceRouteContribution
...
public class WorkspaceMcpFeatureCoordinator(
    ...
    public fun registerInitial(activeServer: Server, features: List<WorkspaceMcpFeature>) {
        features.forEach { register(activeServer, it, sync = false) }
        syncResources(activeServer)
    }

    public fun register(activeServer: Server, feature: WorkspaceMcpFeature, sync: Boolean = true) {
        synchronized(lock) {
    ...
    public fun unregister(activeServer: Server, featureName: String) {
    ...
    public fun syncResources(activeServer: Server) {
